...
class ChessIterableDataset(IterableDataset):
    ...
    def __iter__(self):
                ...
                for sq in chess.SQUARES:
                    ...
                    if color is not None:
                        if color == chess.WHITE:
                            piece_board.append(board_chess.piece_type_at(sq)) # white piece
                        else:
                            piece_board.append(board_chess.piece_type_at(sq) + 7) # black piece
                    else:
                        piece_board.append(0) # no piece

                # append side to move and castle rights, repeat moves
                piece_board.append(0 if board_chess.turn == chess.WHITE else 1)
                piece_board.append(int(board_chess.has_kingside_castling_rights(chess.WHITE)))
                piece_board.append(int(board_chess.has_queenside_castling_rights(chess.WHITE)))
                piece_board.append(int(board_chess.has_kingside_castling_rights(chess.BLACK)))
                piece_board.append(int(board_chess.has_queenside_castling_rights(chess.BLACK)))
                piece_board.append(int(repeat))
                piece_board.append(int(fiftyrule))

                board_array = np.array(piece_board)
                torch_boards.append(board_array)
        ...
        boards_torch = torch.Tensor(torch_boards).long()
